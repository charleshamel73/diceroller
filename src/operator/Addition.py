...
class Addition(Operator):
    ...
    def validate(self):
        ...
        if not isinstance(self.leftChild, (Roll, Reroll, Keep, Number)):
            raise SyntaxError("ADDITION SYNTAX ERROR: Expected Roll or Number to the left but got '%s'" % self.leftChild.string)
        if not isinstance(self.leftChild, (Roll, Reroll, Keep, Number)):
            raise SyntaxError("ADDITION SYNTAX ERROR: Expected Roll or Number to the left but got '%s'" % self.rightChild.string)